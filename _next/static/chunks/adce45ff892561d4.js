...
`,faq:[{question:"Can we just unplug it?",answer:"A superintelligence would likely anticipate that and copy itself to the cloud before you reached the plug."},{question:"Is this real?",answer:"Experts disagree. Leaders at OpenAI and Anthropic treat 'x-risk' (existential risk) seriously, while others think it's sci-fi fearmongering."},{question:"How close are we?",answer:"Some predict AGI by 2029. Others say 2050. The 2023 survey of 2,778 published AI authors put the aggregate at 10% by 2027 and 50% by 2047, and those aggregates have moved earlier in successive surveys."},{question:"Why does the calculator separate timing from severity?",answer:"Because they are different questions and fusing them produces nonsense. A countdown that speeds up when you raise a hostility slider is claiming that a hostile AI arrives sooner, which does not follow, and it implies that anything far enough in the future is automatically safe. Arrival date and outcome are modelled separately here for that reason."},{question:"Why does adding ten points to one slider matter more than another?",answer:"Because the four terms are multiplied. Ten points added to a term sitting at 15% nearly doubles the final answer; the same ten points on a term at 85% changes almost nothing. In a product the smallest link dominates, which is why the tool names it for you. It is usually the thing you are actually arguing about."},{question:"Are the preset numbers what those people actually said?",answer:"The headline figures are: the survey aggregates, and Carlsmith's roughly 5% by 2070. The individual conditionals are not, because nobody publishes those separately. They are one illustrative way of reaching the published total, and the tool says so under each preset."}],relatedCalculators:["alien-communication","time-paradox","conspiracy-theory"]},{id:"alien-communication",title:"Alien Communication Probability",description:"Drake Equation visualizer.",category:"geekgalaxy",slug:"alien-communication",icon:"👽",origin:"A visual deep-dive into the Drake Equation, formulated by Frank Drake in 1961 to estimate the number of active, communicative extraterrestrial civilizations in the Milky Way.",howTo:"Adjust the sliders for star formation, habitable planets, and the 'Great Filter' variables. Watch as the number of potential neighbors in our galaxy shifts from zero to millions.",tips:["The 'L' factor (civilization longevity) is the biggest unknown.","We've only been 'listening' for about 60 years.","The Fermi Paradox asks: 'Where is everybody?'"],tags:["Space","Statistics","Aliens"],content:`
## The Great Silence
...
Warp factors only give you a speed. The crossing time also needs a distance, and distance in the Alpha and Beta Quadrants is not a single line. Qo'noS and Cardassia sit on opposite sides of the Federation, so the run between them is far longer than either world's distance from Earth. Every world on this page carries a position rather than a distance, and the pair you choose is measured.

Four of those positions are real measured stars: Sol, Alpha Centauri at 4.4 light years, 40 Eridani A at 16.5 (Vulcan) and Procyon at 11.4 (Andoria). The rest are fiction and follow the licensed star charts, so treat them as approximate.
...
That sum is your maintenance, the calories you burn in a day. Subtract the deficit your chosen rate needs and you get the daily target the tool leads with. Two guardrails sit on that number: it is never set below 1,500 kcal for men or 1,200 for women, the usual floors for dieting without supervision, and it flags any rate above one percent of your bodyweight a week, where the loss starts taking muscle with the fat.

The projection is not the 3,500-per-pound rule carried in a straight line. It simulates day by day, recomputing your burn from your current weight each morning, because a smaller body costs less to run. On top of that it applies adaptive thermogenesis: after roughly ten percent of bodyweight lost, measured expenditure sits about ten percent below what the new, smaller body predicts. The tool shows both columns side by side so you can see the gap open up. At a 500 kcal daily deficit the flat rule predicts about 52 pounds in a year; this model gives about 24, which is what the published research actually finds. That gap is why month six feels like failure when it is only arithmetic.
...
Let us make it concrete the way the calculator does. Suppose you are 25, you spend 2.5 hours a day on social apps, which is roughly the global average, and you carry that habit to 80.
- Two and a half hours a day across one year is about 913 hours, or 57 waking days.
- Across the 55 years to 80, that is roughly 50,200 hours.
- Divided by a 16-hour waking day, that is about 3,140 waking days: 8.6 years of awake, thumb-moving life.

The division matters. Dividing by 24 instead would report 5.7 years, because it charges a third of the total to hours you were asleep and could never have spent scrolling anyway. Sleep is not yours to reallocate, so it does not belong in the denominator.

Bump the input to four hours a day and the figure climbs past thirteen waking years. Drop it to thirty minutes and it falls under two. The point is not the exact number for your life. It is how violently it swings when you change an input that feels trivial in the moment.
...
Here is the thing a lifetime total cannot tell you, and it is the more useful half.

Nobody scrolls for two and a half hours. They open a feed forty times for four minutes. Those are the same total and they are not the same day, and the difference is exactly the mechanism described at the top of this page. A notification is not aimed at your session length. It is aimed at the count, because raising the count is cheaper, and because an extra re-entry point costs the company nothing.

So the calculator asks for both numbers and puts your sessions on an actual clock. It spreads them across a waking day on an evening-weighted curve, then reads back what is left between them:
- How many feed opens land inside a nine-to-five working block.
- The longest unbroken stretch in that block with no feed open in it.
- How many gaps clear 25 minutes, which is the shortest interval anybody treats as a unit of focused work.

At two and a half hours split forty ways, sixteen opens land inside the working day and the longest clear run is 34 minutes. Split the same two and a half hours ten ways and the longest run is over two hours. The lifetime total is identical in both cases. The day is not.

## How the Math Works
There is no hidden statistical model here, just arithmetic you can check on a napkin.

The ledger runs on multiplication. Daily hours times 365.25, for leap years, gives the annual total. Times the years between your age and the horizon gives the lifetime figure. That divides by 16 rather than 24 to convert into waking days and waking years, for the reason given in the worked example above.

The session model divides. Your daily total divided by your open count gives the average session, and each session is placed on the clock by quantile against a fixed evening-weighted curve rather than drawn at random, so the picture is stable and does not jump when you move an unrelated slider. Counting the sessions that fall between 09:00 and 17:00 gives the interruption count, and the gaps between them give the runs.

The pale band trailing each session in the nine-to-five chart is a recovery window, drawn from the widely cited UC Irvine finding that it takes about 23 minutes to return to an interrupted task. It is used for shading only and never added to any total, because it measures a time to return rather than a quantity of lost output. The headline reading is the longest clear run instead, which is pure arithmetic on the placement, and which keeps moving after the shading has saturated at 100%.

The two cut sliders are targets rather than measured effects, and the page says so. They do different work, which is the useful part. Cutting opens leaves the total roughly intact but restores the shape of a working day. Cutting session length is what moves the lifetime number.
...
`,faq:[{question:"Is screen time actually bad for my brain?",answer:"Yes. Endless scrolling artificially spikes your dopamine. Over time, your baseline dopamine receptors down-regulate, making normal, slower activities (like reading or working) feel excruciatingly boring."},{question:"How much screen time is 'normal'?",answer:"The global average for internet users is nearly 2.5 hours on social media alone. However, 'normal' is not 'healthy'. Aiming for under 30 minutes significantly improves mental wellbeing."},{question:"Why is it so hard to stop?",answer:"Because apps use 'Variable Ratio Reinforcement'. You never know what the next post will be, it might be boring, or it might be amazing. This unpredictability is the most addictive behavioral loop in psychology."},{question:"Does turning my screen to grayscale actually work?",answer:"Absolutely. Bright colors (especially red notification dots) trigger primal attention centers in your visual cortex. Grayscale removes the candy-like appeal of the interface."},{question:"Is this calculator judging me?",answer:"No, it's just doing the math! The goal isn't guilt; the goal is agency. Once you see the true cost of your time, you can make informed decisions about how you spend it."},{question:"Why does it ask how many times I open a feed, not just how long?",answer:"Because the two do completely different damage and the total hides one of them. Two and a half hours split forty ways leaves a longest unbroken stretch of 34 minutes in a working day. The same two and a half hours split ten ways leaves over two hours. Every notification is engineered to raise the count rather than the length, because that is the cheaper one to move."},{question:"Why divide by 16 hours instead of 24?",answer:"Because you cannot scroll during the eight hours you are asleep, so counting them in the denominator quietly flatters the result by about a third. A figure of 8.6 waking years and one of 5.7 calendar years describe the same 50,200 hours. The waking version is the one that answers the question you actually asked."},{question:"Does the 23-minute recovery figure mean I lose 23 minutes per notification?",answer:"No, and the tool is careful not to claim it. The widely cited UC Irvine figure is an average time to get back to the original task, not a measure of time producing nothing. It is used here to shade the picture, never to inflate a total. The headline reading is the longest unbroken stretch you actually get, which is plain arithmetic on where the sessions fall."}],relatedCalculators:["unspent-potential","procrastination-roi","life-visualized"]},{id:"coffee-code",title:"Coffee to Code Efficiency",description:"Calculate caffeine-fueled productivity.",category:"lifehacks",slug:"coffee-code",icon:"☕",origin:"Computer science legend says that a programmer is a machine for turning coffee into code. This tool calculates the 'Goldilocks Zone' of caffeination for maximum output.",howTo:"Add the drinks you actually had and the times you had them, then set where your own peak sits and how fast you clear caffeine. The tool plots your focus across the whole day, marks the window worth spending on hard work, and tells you how much is still on board at bedtime.",tips:["Hydrate with water between cups of coffee.","Coffee-fueled productivity peaks about 45 minutes after ingestion.","Avoid caffeine 6 hours before your intended sleep time."],tags:["Productivity","Work"],content:`
...
The far side of that hill is deliberately gentler than the near side. Being 190 milligrams over your peak costs you less than being 190 milligrams under it, because caffeine is still blocking adenosine even while it is making you twitchy. Dana's own numbers say so: her no-coffee morning produced 3 finished tasks and her four-coffee morning produced 5, against a best of 8. The curve here is fitted to exactly those four points.

The curve also has a clock, which is the part most caffeine advice leaves out. Caffeine is not a level you set, it is a dose that rises and then falls, so both halves are modelled. Absorption is quick: a drink reaches its peak in your blood about 45 minutes after you finish it. Clearance is slow: the half-life is roughly five hours for most adults, which is why the coffee that felt harmless at 4pm is still a third present at 9pm. Smoking roughly halves that figure, and oral contraceptives or pregnancy can double it. Running both curves together means the tool works from the caffeine actually on board at each moment rather than the total you drank, and reads your focus off that.
...
## How the Math Works
The calculator takes three numbers off you and hands back dates rather than totals.

The first is the leak: the hours that drain away on an average day. That gets multiplied by 365, because the leak does not take weekends off, and the result is the bill. Three hours a day is 1,095 hours a year, which is the number nobody puts on a balance sheet.

The second is what you would honestly redirect, and it is deliberately a separate number. Nothing above is asking you to reclaim the lot. Keeping most of it as real, guilt-free rest and pointing one hour somewhere is the trade that actually holds.

The third is the one most tools quietly skip: how many days a week you would genuinely sit down. Seven out of seven is a fantasy, and assuming it is exactly how a single idle hour becomes fifty new skills a year on paper and nothing at all in practice. One hour on six days banks 312 hours in a year. Three hours on one day banks 156. Cadence moves the line further than intensity does.

Multiply the redirected hours by the days you show up and you have a weekly rate. Divide each milestone's hour cost by that rate and you have the week you reach it, which becomes a calendar date.

### The ladder, and the rungs you do not reach
Each pursuit carries its own ladder of four milestones at published hour costs, rather than one generic scale applied to everything. A new language is pinned to the Foreign Service Institute's figure of 600 to 750 class hours for Spanish, French or Italian. Programming is pinned to the 500 to 900 hours a full-time bootcamp runs. Sewing is pinned to the worked example above: a skirt, then commissions, then a wardrobe.

Milestones your line does not cross inside twelve months are not hidden. They stay on the chart, above where the line stops, and each one reports the daily figure that would bring it inside the year. Learning that guitar mastery wants three hours a day rather than one is more useful than being told you could start fifty-four new skills.

### Where the calendar overrules you
One pursuit refuses to be bought with hours. Running is capped by adaptation rather than effort: tendon and bone remodel on a schedule of their own, and stacking long runs into a fortnight produces an injury rather than a half marathon. So each running milestone also carries a floor in weeks, taken from the length of the standard plan, and the tool never returns a date earlier than that floor allows. Where the floor is what binds, the chart draws the gap between the hours you banked and the date the plan permits.

The tax is never about shame. It is about visibility. Once you can see the size of the bill and the date on the other side of it, you get to decide, hour by hour, whether to keep paying it or to spend that potential on purpose.
...
`,faq:[{question:"Why does the calculator ask how many days a week, rather than just hours a day?",answer:"Because that is the variable that decides whether any of it happens. One hour across six days banks 312 hours in a year; three hours on a single day banks 156, despite feeling like the more serious commitment. Assuming seven days out of seven is how a calculator ends up promising fifty new skills a year that nobody has ever acquired."},{question:"Why does it only let me spend the time on one thing?",answer:"Because the hours are not reusable. It is tempting to report the same idle time as dozens of skills started, a hundred books read and thousands of dollars earned all at once, but you get to spend an hour exactly once. Picking one pursuit is the honest version of the question."},{question:"Where do the hour figures come from?",answer:"Each ladder is pinned to a published source rather than a generic scale, and the tool names its source under every result. The Foreign Service Institute puts Spanish, French and Italian at 600 to 750 class hours to professional working proficiency. Full-time coding bootcamps run 12 to 24 weeks at roughly 40 hours, which is where the 500 to 900 hour band comes from. The running plans supply their own lengths."},{question:"Why can I not reach a half marathon faster by training more hours a day?",answer:"Because that milestone is capped by adaptation, not by effort. Tendon, bone and connective tissue remodel on a schedule you do not control, and compressing a plan produces an injury rather than a finish line. Running milestones carry a floor in weeks, and the calculator will not return a date earlier than the plan allows however many hours you pour in."},{question:"Is it worth redirecting time if I cannot reach the top rung this year?",answer:"Almost always, yes, because the ladder is steepest at the bottom. The first rung of most pursuits costs around twenty hours and the top one costs hundreds. Clearing rungs one and two inside a year is a real change in what you can do, and it is the part of the curve your reclaimed hours actually buy."},{question:"What if I do not want to reclaim the time at all?",answer:"Then do not. Deliberate, guilt-free rest is one of the highest-value uses of an hour there is, which is why redirected hours are a separate slider from the leak. The tax is specifically about the in-between hours that are neither restful nor productive, the ones that leave you more depleted than before."}],relatedCalculators:["procrastination-roi","time-is-money","social-media-addiction"]},{id:"stress-ice-cream",title:"Stress-to-Ice-Cream Ratio",description:"Emotional support dairy calculator.",category:"lifehacks",slug:"stress-ice-cream",icon:"🍦",origin:"Sometimes, the only solution to a bad day is a pint of mint chocolate chip. This scientific (ish) scale matches your current stress level to the appropriate serving size.",howTo:"Rate your day on a scale of 'Minor Inconvenience' to 'Total Existential Crisis', then flag a recent breakup or a looming deadline. You get a serving in millilitres with the calories attached, capped at half a pint, and a reading of what the inputs say about your stress load rather than just your dessert.",tips:["Share a pint to halve the stress and double the joy. Social contact lowers stress hormones directly, so this one is real.","Serve it into a bowl rather than eating from the carton. Same dopamine, no autopilot.","If the serving comes back capped, that is the tool telling you the stress reading ran past the point where more ice cream helps.","A waffle cone adds +10 to your happiness stat. This one is not real."],tags:["Mental Health","Food"],content:`
...
## How the Math Works
The calculator combines your three inputs into a single recommended serving, in millilitres, because a pint is not a serving and never was. In plain terms:
- serving equals base stress level, adjusted up by the heartbreak bump, then multiplied by the deadline factor, then held to the ceiling.
- Base Stress Level is your 1-to-10 rating and does the heavy lifting. It runs from 40 ml at a one, which is a couple of spoonfuls, to 220 ml at a ten.
- The Heartbreak Bump adds a flat 60 ml when a recent breakup is flagged, reflecting the real biological pull toward high-fat, high-sugar food during emotional distress. It adds, because that pull is a fixed one.
- The Deadline Multiplier scales the whole result by 1.25 when work stress is in play, because acute deadline pressure amplifies whatever craving is already there rather than adding a separate one. That distinction is the reason the two flags are modelled differently instead of both just adding a number.
- The Goldilocks ceiling holds the answer at 236 ml, half a US pint, which is roughly two scoops and about 500 calories of premium ice cream.

The ceiling is not decoration. Comfort eating genuinely works, and it stops scaling: past a generous bowl you are trading tonight's stress for tomorrow's sugar crash, so the number stops rather than rewarding a worse evening with a bigger serving.

Watch what the worked example above does with it. A stress of 8 gives a base of 180 ml. The breakup adds 60, taking it to 240. The deadline multiplies that by 1.25, asking for 300. The ceiling holds it at 236, and the tool draws the 64 ml it wanted and did not get as a pale band above the line. That overshoot is not a portion you missed out on. It is the reading.

### The serving is a stress gauge
Which is the part worth taking away. When the formula runs past its own ceiling, the number has stopped being about dessert and started being about load. An 8-out-of-10 night stacked with a breakup and a deadline is not asking you to optimise a portion size, and the tool says so plainly rather than quietly handing back a bigger figure. That is why the alternatives sit next to the answer instead of at the bottom of an article nobody scrolls to.
...
Now look at what those two figures actually are. A cayenne's published band runs 30,000 to 50,000, and a habanero's runs 100,000 to 350,000. Pick the mildest habanero and the hottest cayenne and the ratio is 2. Pick the hottest habanero and the mildest cayenne and it is nearly 12. The 6.7 is a fair middle, and it is a middle rather than a measurement. The calculator reports the midpoint figure with that spread underneath it, because a single number here implies a precision the source data does not have.

## Why the Ruler Is Logarithmic
Plot these peppers on an ordinary scale, one where the distance from 0 to 100,000 is the same as from 100,000 to 200,000, and something unhelpful happens. Against a Pepper X at 2.69 million, a bell pepper, a pepperoncini, a poblano, a jalapeno and a serrano all land inside the first one percent. Five distinct peppers, covering the entire range most people ever cook with, drawn as the same indistinguishable sliver at the left edge.

So the ruler on this page is logarithmic. Each gridline is ten times the one before it: 100, then 1,000, then 10,000, and so on up to 10 million. That spreads the mild end out far enough to see, and it turns the cliff between a jalapeno and a habanero into a gap you can actually measure with your eye.

It also puts two things on the same ruler that are not food. Police-grade pepper spray occupies a band from roughly 2 million to 5.3 million SHU, and pure capsaicin sits at about 16 million. Look at where the Carolina Reaper falls against that spray band and the warning labels stop looking like marketing.
...
That last line is worth taking seriously, because instinct and folklore both point the wrong way. A cold beer with something spicy is a fine thing to drink. It is not a remedy, and reaching for it as one just moves the oil around.
